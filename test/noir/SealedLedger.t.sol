...
contract SealedLedgerTest is Test {
    ...
    function config() internal view returns (NoirRankedShares.Config memory) {
        return NoirRankedShares.Config({
            forwarder: forwarder,
    ...
    function test_proposalsKeepNoirProfileLimitsAndBindContent() public {
        ...
        vm.prank(alice);
        pool.propose(ref, bytes32(0), 5, recipientA);
        vm.startPrank(owner);
    ...
    function test_constructorRejectsBadConfig() public {
        ...
        NoirRankedShares.Config memory cfg = config();
        ...
        new NoirRankedShares(token, owner, DEADLINE, config());
    }

    function expectBadConfig(NoirRankedShares.Config memory cfg) internal {
        vm.expectRevert(NoirRankedShares.InvalidConfig.selector);
        new NoirRankedShares(token, owner, DEADLINE, cfg);
    }

    function test_addProjectEnforcesCostAndCount() public {
        vm.startPrank(owner);
        vm.expectRevert(NoirRankedShares.ZeroCost.selector);
        pool.addProject(0, recipientA);
        vm.expectRevert(NoirRankedShares.CostTooLarge.selector);
    ...
    function test_openVotingRequiresProjects() public {
        vm.prank(owner);
        vm.expectRevert(NoirRankedShares.NoProjects.selector);
        pool.openVoting();
    }
    ...
    function test_depositsCapTotalWeight() public {
        ...
        vm.prank(alice);
        vm.expectRevert(NoirRankedShares.WeightOverflow.selector);
    ...
    function test_directBallotRequiresMinimumWeightAndValidRanks() public {
        ...
        vm.expectRevert(NoirRankedShares.BelowMinimumVote.selector);
        pool.vote(hex"0102");
        pool.contribute(5 * USDC);
        vm.expectRevert(NoirRankedShares.InvalidBallot.selector);
    ...
    function test_sealedVoteRequiresSeatWeightAndValidPoint() public {
        openWithProjects();
        vm.prank(alice);
        vm.expectRevert(NoirRankedShares.NoSeatWeight.selector);
        pool.voteSealed(GX, GY, 5);
        vm.prank(org);
        pool.sponsor(60 * USDC, one(alice));
        vm.startPrank(alice);
        vm.expectRevert(NoirRankedShares.InvalidCiphertext.selector);
        ...
        emit NoirRankedShares.SealedVote(alice);
        pool.voteSealed(GX, GY, 5);
        vm.stopPrank();
    ...
    function test_zeroWeightCannotVoteWithMinimumsOff() public {
        ...
        cfg.minDirectVote = 0;
        cfg.minSealedVote = 0;
        NoirRankedShares p = new NoirRankedShares(token, owner, DEADLINE, cfg);
        vm.startPrank(owner);
    ...
    function test_sealedVoterCap() public {
        ...
        vm.prank(members[8]);
        vm.expectRevert(NoirRankedShares.TooManySealedVoters.selector);
    ...
    function test_closingPhaseAfterDeadlineBlocksVotes() public {
        ...
        pool.sponsor(60 * USDC, one(alice));
        vm.warp(DEADLINE);
        assertEq(uint256(pool.phase()), uint256(NoirRankedShares.Phase.Closing));
