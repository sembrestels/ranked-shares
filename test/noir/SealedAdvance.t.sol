...
contract SealedAdvanceTest is FixtureLoader {
    ...
    function test_ingestChainFollowsCheckpoints() public {
        ...
        for (uint256 k = 0; k < n; k++) {
            ...
            emit NoirRankedShares.Ingested(k);
            pool.advance("", ingestInputs(k), false);
    ...
    function test_ingestRejectsOutOfOrderAndTampered() public {
        ...
        vm.expectRevert(NoirRankedShares.ProofOutOfOrder.selector);
        pool.advance("", second, false);
        bytes32[] memory first = ingestInputs(0);
        first[7] = bytes32(uint256(first[7]) + 1); // hOut
        vm.expectRevert(NoirRankedShares.InputMismatch.selector);
    ...
    function test_ingestRejectsWhenVerifierRejects() public {
        ...
        vm.expectRevert(NoirRankedShares.InvalidProof.selector);
    ...
    function test_tallyRequiresIngestAndReport() public {
        ...
        ingestAll();
        vm.expectRevert(NoirRankedShares.TranscriptPending.selector);
        pool.advance("", t0, false);
    }
    ...
    function test_tallyChainFinalizes() public {
        ...
        for (uint256 g = 0; g + 1 < n; g++) {
            pool.advance("", tallyInputs(g), false);
            assertEq(uint256(pool.finality()), uint256(NoirRankedShares.Finality.None));
        }
        uint256[] memory order = fxUintArray(".funded");
        vm.expectEmit(false, false, false, true);
        emit NoirRankedShares.Finalized(NoirRankedShares.Finality.Proven, order);
        pool.advance("", tallyInputs(n - 1), false);
        assertEq(uint256(pool.finality()), uint256(NoirRankedShares.Finality.Proven));
        assertEq(uint256(pool.phase()), uint256(NoirRankedShares.Phase.Done));
    ...
    function test_finalProofChecksTranscriptHashAndResult() public {
        ...
        vm.expectRevert(NoirRankedShares.TranscriptMismatch.selector);
        pool.advance("", last, false);
        last = tallyInputs(n - 1);
        last[5] = bytes32(uint256(last[5]) + 1); // fundedCount
        vm.expectRevert(NoirRankedShares.ResultMismatch.selector);
        pool.advance("", last, false);
    }
    ...
    function test_restartTallyResetsToIngestedState() public {
        ...
        emit NoirRankedShares.TallyRestarted();
    ...
    function test_restartByOthersReverts() public {
        ...
        address stranger = makeAddr("stranger");
        vm.prank(stranger);
        vm.expectRevert(NoirRankedShares.NotCoordinator.selector);
        pool.advance("", tallyInputs(0), true);
        assertEq(pool.stateCommit(), before);
    }
    ...
    function test_revertingVerifierSurfacesAsInvalidProof() public {
        ...
        NoirRankedShares p2 = deployWith(address(bad), address(bad), later);
        vm.startPrank(owner);
    ...
    function deployWith(address ingest, address tally, uint64 deadline) internal returns (NoirRankedShares) {
        uint256[] memory pk = fxWords(".pk");
        NoirRankedShares.Config memory cfg = NoirRankedShares.Config({
            forwarder: forwarder,
        ...
        return new NoirRankedShares(new MockERC20(), owner, deadline, cfg);
    }
    ...
    function test_acceptProvisionalAfterGrace() public {
        report();
        vm.expectRevert(NoirRankedShares.ProofPending.selector);
        pool.acceptProvisional();
        vm.warp(DEADLINE + 1 days);
        pool.acceptProvisional();
        assertEq(uint256(pool.finality()), uint256(NoirRankedShares.Finality.Attested));
        assertEq(pool.fundedProjects().length, fxUintArray(".funded").length);
    }

    function test_acceptProvisionalNeedsAReport() public {
        vm.warp(DEADLINE + 1 days);
        vm.expectRevert(NoirRankedShares.ResultPending.selector);
        pool.acceptProvisional();
    }
    ...
    function test_abandonGraceArithmeticDoesNotOverflow() public {
        ...
        NoirRankedShares p2 = new NoirRankedShares(tok2, owner, laterDeadline, cfg);
        vm.startPrank(owner);
        p2.addProject(1, recipient);
        p2.openVoting();
        vm.stopPrank();
        ...
        p2.abandon();
        assertEq(uint256(p2.finality()), uint256(NoirRankedShares.Finality.Abandoned));
    }
    ...
    function test_abandonFromClosingWithoutClose() public {
        ...
        pool.abandon();
        assertEq(uint256(pool.finality()), uint256(NoirRankedShares.Finality.Abandoned));
        assertEq(uint256(pool.phase()), uint256(NoirRankedShares.Phase.Done));
    ...
    function test_abandonRefusedWhenReported() public {
        report();
        vm.warp(DEADLINE + 7 days);
        vm.expectRevert(NoirRankedShares.ResultAlreadyReported.selector);
        pool.abandon();
    }
