...
///      `test/noir/verifiers/RealProofs.t.sol` runs the same shapes against the generated
...
contract AdvanceManyTest is FixtureLoader {
    ...
    function test_batchEmitsTheSameEventsAsOneAdvanceEach() public {
        ...
        for (uint256 g = 0; g < nT; g++) {
            ...
            emit NoirRankedShares.Advanced();
        }
        vm.expectEmit(false, false, false, true);
        emit NoirRankedShares.Finalized(NoirRankedShares.Finality.Proven, fxUintArray(".funded"));
        pool.advanceMany(proofs, pis, false);
    }
    ...
    function test_emptyBatchReverts() public {
        vm.expectRevert(NoirRankedShares.EmptyBatch.selector);
    ...
    function test_aProofAfterTheFinalisingOneReverts() public {
        ...
        pool.advanceMany(proofs, pis, false);
        assertEq(uint256(pool.finality()), uint256(NoirRankedShares.Finality.None), "the whole batch rolled back");
    }
