...
contract SealedReportTest is FixtureLoader {
    ...
    function test_onlyForwarder() public {
        vm.expectRevert(NoirRankedShares.NotForwarder.selector);
        pool.onReport("", abi.encode(uint8(2), abi.encode(uint256(1))));
    }

    function test_unknownKind() public {
        vm.prank(forwarder);
        vm.expectRevert(NoirRankedShares.UnknownReport.selector);
    ...
    function test_resultAcceptedOnceWithMatchingRootAndTranscriptHash() public {
        ...
        emit NoirRankedShares.ProvisionalResult(order);
        vm.expectEmit(false, false, false, true);
        emit NoirRankedShares.Transcript(transcript);
    ...
    function test_resultRejectsInvalidFundedList() public {
        ...
        order[1] = 0;
        vm.prank(forwarder);
        vm.expectRevert(NoirRankedShares.InvalidResult.selector);
    ...
    function test_resultRejectsFundedOrderOverBudget() public {
        ...
        NoirRankedShares small = new NoirRankedShares(token2, owner, DEADLINE, cfg);
        vm.startPrank(owner);
    ...
    function test_resultRejectsMalformedTranscript() public {
        ...
        vm.prank(forwarder);
        vm.expectRevert(NoirRankedShares.InvalidTranscript.selector);
