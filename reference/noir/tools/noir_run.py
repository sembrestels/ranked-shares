...
Usage, from `reference/`: python3 -m noir.tools.noir_run --fixture test_main [--only ingest|tally]
...
from noir.tools.noir_inputs import load

HERE = os.path.dirname(os.path.abspath(__file__))
REFERENCE = os.path.join(HERE, "..", "..")
ROOT = os.path.join(REFERENCE, "..")
...
def run_one(name, fx, kind, index):
    ...
    try:
        subprocess.run(
            [sys.executable, "-m", "noir.tools.noir_inputs", "--fixture", name, f"--{kind}", str(index), "--out", prover],
            check=True, capture_output=True, cwd=REFERENCE,
        )
    except subprocess.CalledProcessError as e:
        print(f"FAIL {name} {kind} {index}: noir_inputs exited {e.returncode}")
