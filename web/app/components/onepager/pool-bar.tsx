...
export function PoolBar({ funded, budget, label }: { funded: readonly number[]; budget: number; label: string }) {
  const spent = funded.reduce((sum, id) => sum + PROPOSALS[id].cost, 0);
  return (
    <div className="op-pool" role="img" aria-label={`${label}: ${funded.map((id) => `${PROPOSALS[id].title} ${usd(PROPOSALS[id].cost)}`).join(", ") || "nothing funded"}${budget > spent ? `, ${usd(budget - spent)} unspent` : ""}`}>
      {funded.map((id) => (
        <span key={id} className="op-pool-seg" data-camp={PROPOSALS[id].camp} style={{ flexGrow: PROPOSALS[id].cost }}>
          <span className="op-pool-name">{PROPOSALS[id].short}</span>
          <span className="op-pool-cost">{usd(PROPOSALS[id].cost / 1000)}k</span>
        </span>
      ))}
